 ...
 *   - == Section == headers
 *   - > notes, rendered as commentary rather than numbered steps
...
export function slugify(value) {
  return String(value)
    ...
    .replace(/[\u0300-\u036f]/g, "") // drop accents: jalapeno, not jalape-o
    .replace(/[\u2018\u2019\u02bc\u2032']/g, "") // apostrophes: Tsos, not Tso-s
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
...
/**
 * Comments become "note" blocks rather than being discarded, so asides can be
 * rendered as commentary instead of numbered steps.
 *
 *   [- a block comment -]   -> note
 *   -- a whole-line comment -> note
 *   step text -- trailing   -> trailing part dropped from the step
 */
function extractNotes(source) {
  const notes = [];
  const withoutBlocks = source.replace(/\[-([\s\S]*?)-\]/g, (_, body) => {
    const text = body.trim();
    if (text) notes.push(text);
    return "";
  });
  return { text: withoutBlocks, notes };
}

function splitLineComment(line) {
  const match = line.match(/^\s*--\s?(.*)$/);
  if (match) return { note: match[1].trim(), rest: "" };
  return { note: "", rest: line.replace(/\s--\s?.*$/, "") };
}
...
export function parseRecipe(source) {
  ...
  const sections = [];
  let current = { name: "", blocks: [] };

  for (const rawBlock of body.split(/\r?\n\s*\r?\n/)) {
    if (!rawBlock.trim()) continue;

    const { text: withoutBlocks, notes: blockNotes } = extractNotes(rawBlock);
    blockNotes.forEach((note) =>
      current.blocks.push({ type: "note", text: note })
    );
    ...
    const flushNote = () => {
      ...
      if (text) current.blocks.push({ type: "note", items: parseStep(text) });
      noteLines = [];
    };

    for (const line of withoutBlocks.split(/\r?\n/)) {
      const { note, rest } = splitLineComment(line);
      if (note) current.blocks.push({ type: "note", items: parseStep(note) });
      if (!rest.trim()) continue;

      // "> ..." is a CookLang note: commentary, not a numbered step.
      const asNote = rest.match(/^\s*>\s?(.*)$/);
      if (asNote) {
        noteLines.push(asNote[1].trim());
        continue;
      }
    ...
    if (heading && text.startsWith("=")) {
      if (current.blocks.length || current.name) sections.push(current);
      current = { name: heading[1].trim(), blocks: [] };
      continue;
    }

    current.blocks.push({ type: "step", items: parseStep(text) });
  }
  if (current.blocks.length || current.name) sections.push(current);
